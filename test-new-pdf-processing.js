// Test script for new PDF processing with pdfjs-dist
const fs = require('fs')
const path = require('path')

async function testNewPdfProcessing() {
  try {
    console.log('🧪 Testing new PDF processing with pdfjs-dist...\n')
    ...
    if (response.ok) {
      ...
      if (result.data?.content) {
        ...
        const { DocumentProcessorService } = require('./lib/services/document-processor.service.ts')
        const chunks = DocumentProcessorService.splitIntoChunks(result.data.content, 500)
