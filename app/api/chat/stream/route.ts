...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.apiRequest('POST', '/api/chat/stream', userId)
    ...
    if (lastUserMessage) {
      ...
      try {
        ...
        try {
          ...
          console.log(`[Stream Chat API] 📄 Fallback search - Document context length: ${documentContext.length}`)
        } catch (fallbackError) {
          console.error('[Stream Chat API] ❌ Fallback document search also failed:', fallbackError)
          documentContext = ''
          searchResults = null
        }
      }
      ...
      if (imageUrl) {
        console.log(`[Stream Chat API] Image detected for multimodal message: ${imageUrl}`)
      }
    }
      ...
      if (!effectiveUserId) {
        ...
        try {
          ...
          if (!defaultUser) {
            ...
            console.log('[Stream Chat API] Created default widget user:', defaultUser.id)
          }
          
          effectiveUserId = defaultUser.id
        } catch (error) {
          console.error('[Stream Chat API] Failed to create/find default user:', error)
      ...
      async start(controller) {
        try {
          ...
          const savedMessage = await ConversationService.createMessage({
            ...
            tokensUsed: streamResponse.usage?.total_tokens,
          ...
          try {
            ...
            await ServerAnalyticsService.updateAnalyticsForMessage(
              ...
              streamResponse.usage?.total_tokens,
              responseTime
            )
          } catch (error) {
          ...
          const completionData = {
            ...
            data: {
              ...
              model: streamResponse.model,
              finish_reason: streamResponse.finish_reason,
          ...
          console.error('[Stream Chat API] Error in stream:', error)
