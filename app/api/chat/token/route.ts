...
import { validateToken, requirePermission, createTokenAuthResponse } from '@/lib/middleware/token-auth'
import type { OpenAIMessage } from '@/lib/openai-api'

export async function POST(request: NextRequest) {
  try {
    // Validate token authentication
    const tokenAuth = await validateToken(request)
    if (!tokenAuth.isValid) {
      return createTokenAuthResponse(tokenAuth.error || 'Invalid token')
    }

    // Check if token has chat permission
    if (!requirePermission(tokenAuth.permissions || [], 'chat')) {
      return createTokenAuthResponse('Insufficient permissions for chat', 403)
    }

    const body = await request.json()
    let { messages, message, botConfig, conversationId } = body

    // Use botId from token auth
    const botId = tokenAuth.botId!
    const userId = tokenAuth.userId!

    logger.apiRequest('POST', '/api/chat/token', parseInt(userId))
    ...
    if (lastUserMessage) {
      ...
      try {
        console.log(`[Chat API Token] 🔍 Searching documents in Pinecone for bot ${botId} with message: "${messageText}"`)
        ...
        if (pineconeResults.length > 0) {
          console.log(`[Chat API Token] ✅ Found ${pineconeResults.length} relevant document chunks in Pinecone`)
          ...
          console.log(`[Chat API Token] 📄 Document context length: ${documentContext.length} characters`)
        } else {
          console.log(`[Chat API Token] ⚠️ No relevant documents found in Pinecone for query: "${messageText}"`)
          documentContext = ''
          searchResults = null
        }
      } catch (error) {
        console.error('[Chat API Token] ❌ Pinecone document search failed, falling back to traditional search:', error)
        ...
        try {
          ...
          console.log(`[Chat API Token] 📄 Fallback search - Document context length: ${documentContext.length}`)
        } catch (fallbackError) {
          console.error('[Chat API Token] ❌ Fallback document search also failed:', fallbackError)
          documentContext = ''
          searchResults = null
        }
      }
      ...
      if (config.chat.useVectorSearch) {
        try {
          console.log(`[Chat API Token] 🔍 Searching conversation context for bot ${botId}, user ${userId}`)
          ...
          if (relevantMessages.length > 0) {
            ...
            console.log(`[Chat API Token] Found ${relevantMessages.length} relevant conversation messages`)
          } else {
            console.log(`[Chat API Token] No relevant conversation context found from vector search`)
            ...
            if (conversationId) {
              try {
                ...
                if (recentMessages.length > 0) {
                  ...
                  console.log(`[Chat API Token] Found ${recentMessages.length} recent messages from database`)
                }
              } catch (dbError) {
                console.error('[Chat API Token] Failed to get recent messages from database:', dbError)
              }
            }
          }
        } catch (error) {
      ...
      if (imageUrl) {
        console.log(`[Chat API Token] Image detected for multimodal message: ${imageUrl}`)
      }
    }
...
- Average relevance score: ${searchResults.summary.averageScore}`
    ...
    if (!currentConversationId) {
      const conversation = await ConversationService.createConversation({
        botId,
        userId: userId,
        title: 'Token API Conversation',
        isTest: true // Mark token API conversations as test conversations
      })
    ...
    if (config.chat.useVectorSearch) {
      try {
        ...
        console.log(`[Chat API Token] Messages stored in Pinecone for conversation ${currentConversationId}`)
      } catch (error) {
    ...
    logger.apiResponse('POST', '/api/chat/token', 200, responseTime)
    ...
    logger.apiError('POST', '/api/chat/token', error as Error)
